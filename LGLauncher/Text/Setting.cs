...
namespace LGLauncher
{
	...
	public static class PathList
	{
		...
		public static bool Make()
		{
			...
			//　複数ある場合の優先順位は、
			//　　（高）　Appフォルダの .exe .vbs .js、SystemDirの .exe .vbs .js　（低）
			var lsPathList = new string[]{
																Path.Combine(AppDir, "LogoSelector.exe"),
																Path.Combine(AppDir, "LogoSelector.vbs"),
																Path.Combine(AppDir, "LogoSelector.js"),
