...
namespace LGLauncher
{
	static partial class EditFrame
	{
		public static bool Concat()
		{
			...
			//ファイル名からフレーム数取得
			int beginFrame = int.MaxValue, endFrame = int.MaxValue;					//デバッグのためにint.MaxValueを初期値にする。
			if (1 <= PathList.No)
			{
			...
			editList = EditFrame.FlatOut_Main(editList, 14.0);
			...
			//var chapTextList = new List<string>() { chapText };
			//if (Directory.Exists(PathList.ChapDir))																		//フォルダがある？
			//	TextW.WriteAllLines(outChapPath, chapTextList, TextEnc.UTF8_bom);				//BOMつきUTF8
